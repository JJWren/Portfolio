...
## Coverage (1085 tests, 62 fixtures — as of the Unit 12 phase 12b PR #114 review remediation: TRUSTED_PROXIES fails startup when configured but every entry is invalid, the sign-in path prefix match is case-insensitive, SubmissionLimiter's Sweep/Allow/RetryAfter race is closed with a lock-and-verify protocol, and hit expiry is >= Window consistently — on top of Unit 12b's rate limiting for auth, feeds, comments and reports: framework policies on the auth group, the feeds and the counted redirects, a path-scoped global limiter for the OAuth handler callback paths, the generalized circuit limiter for comments and reports, and TRUSTED_PROXIES for the forwarded-for boundary — on top of Unit 12a's security headers, Unit 11, the current belt for the rank bar, the Daily visitors tile quick fix (PR 1.5) and Unit 14, the admin stats daily-visitors chart)
| Area | Fixtures |
|---|---|
