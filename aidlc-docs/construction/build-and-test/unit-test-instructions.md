...
## Coverage (1066 tests, 61 fixtures — as of Unit 12b, rate limiting for auth, feeds, comments and reports: framework policies on the auth group, the feeds and the counted redirects, a path-scoped global limiter for the OAuth handler callback paths, the generalized circuit limiter for comments and reports, and TRUSTED_PROXIES for the forwarded-for boundary — on top of Unit 12a's security headers, Unit 11, the current belt for the rank bar, the Daily visitors tile quick fix (PR 1.5) and Unit 14, the admin stats daily-visitors chart)
| Area | Fixtures |
|---|---|
