...
- **BR-14 Motion.** Every new `animation` and `transition` in the landing CSS is listed in the single `prefers-reduced-motion` block of `app.css`, and every declaration in that block carries `!important` so the override beats any later or more specific animation rule (the Phase 3 review found that without it the block never won the cascade); the mobile scroll glow lives under `@supports (animation-timeline: view())` with the static row as the fallback. Timeline names are index-based (`era-1` to `era-8`) so two eras on the same belt never share a timeline. Eight rather than `MaxEras` (12) because the named timelines are enumerated by hand in CSS, one rule pair per index: the realistic ladder is five or six eras, eight leaves headroom without twelve rule pairs, and rows past the eighth keep the anonymous `view()` row glow, so only the per-row band glow stops.
