...
- [x] Owner content entry after deploy: set `SITE_FLAVOR=bjj` in the production `.env`, then paste the Phase 2 rows of the content sheet at `/admin/site` (hero heading, eyebrow, tagline, game plan, belt caption, degrees, principles).
...
- [x] Owner content entry after deploy: paste the eras and now rows of the content sheet at `/admin/site`.
...
- [x] Gate: build `-warnaserror` and `dotnet test` green (0 warnings, 689 tests); five-area internal review (security and framework clean; four small notes applied: mat image fetchpriority low, honest temp-file assertions, in-file note on the desk tile, editor hint); Copilot round 1 with one doc-comment note fixed, round 2 with zero comments; PR #94 squash-merged 2026-09-04 as `18b1788`; plan, state and audit updated.
- [x] Owner actions after deploy: set `OWNER_PHOTO_FLIP_FILE` (a path on the same read-write mount as the primary photo), upload the mat portrait and paste both alt texts at `/admin/site`.
...
- [x] Optional, owner's call: under `Bjj`, the theme toggle's `title` reads "Switch to the white gi (light theme)" / "Switch to the black belt (dark theme)" (set by `theme.js` from a `data-flavor` attribute on `<html>`); the `aria-label` stays functional. Deferred: the owner has not chosen the wording, so the title stays as it is until asked (recorded in audit.md at the Phase 5 close-out). Done 2026-09-05 as the Unit 10 addendum (PR #101, 9d97fd1): the tooltip reads Switch to the white gi (light theme) while dark and Switch to the black gi (dark theme) while light (the owner corrected belt to gi at approval); plan construction/plans/unit10-addendum-tooltip-plan.md.
