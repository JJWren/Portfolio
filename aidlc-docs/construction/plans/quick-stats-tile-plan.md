...
- [x] Push; PR `fix: show the average per day on the daily visitors tile`; Copilot gate per CONTRIBUTING.md; squash-merge; realign master stash-first; delete the branch; tick this plan, the state file, the audit and memory; report to the owner (no stop). Done 2026-09-13: PR #107 (commits ff9aa80 fix and 6720770 docs); CI green (build-test, pr-title, CodeQL, GitGuardian); Copilot reviewed with zero comments, a clean pass; squash-merged as e79bf49; master realigned (level with origin, nothing to stash); local branch deleted. Release rides with Unit 11 and 12a (owner decision Q2).
