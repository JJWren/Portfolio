...
- **Merge**: `env -u GITHUB_TOKEN gh pr merge N --squash --subject "<PR title> (#N)" --body "..."`; the remote branch is auto-deleted. Then realign, IN THIS ORDER: `git stash push aidlc-docs/audit.md` (and any other uncommitted orchestrator docs), `git switch master`, `git fetch origin`, `git reset --hard origin/master`, `git stash pop`, `git branch -D <branch>`. Never skip the stash: with an uncommitted audit append, `git switch` aborts and a following `reset --hard` silently discards the append (this happened twice on 2026-09-04; three entries had to be restored from the session record).
