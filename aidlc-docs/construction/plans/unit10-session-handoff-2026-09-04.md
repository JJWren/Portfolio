...
- **Hook noise**: a local Claude plugin hook (`security_reminder_hook.py`, file missing) fires on every commit, push and assistant turn and reports "found issues" with no findings. It is a machine-level plugin fault; reinstalling that plugin should stop it. Ignore it otherwise, but note that each acknowledgment consumes context.
