...
- [x] After the merge: release-please proposes the next minor; the owner bumps the compose image tag in `Z:\docker\portfolio\docker-compose.yml` and recreates the container (the two commands from the checklist's Step 0). Release 1.26.0 cut 2026-09-05 (PR #100, 88f3938). Remaining, the owner: bump the compose image tag to 1.26.0 and recreate the container. Deployed: the owner bumped the compose tag to 1.26.0 and recreated the container on 2026-09-05 (running 1.26.0, healthy; the live page serves the pinned header).
