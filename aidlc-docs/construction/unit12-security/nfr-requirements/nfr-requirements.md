...
| `X-Frame-Options` | `DENY` | Clickjacking in a browser without CSP frame-ancestors. The framework's antiforgery default `SAMEORIGIN` is suppressed in 12a so `DENY` is the only value the app sends; a proxy copy, if one exists, is harmless: frame-ancestors governs where both exist, and a conflict falls back to DENY in Chrome. | Not set by the app's code; emitted by antiforgery's default until 12a. |
| `Permissions-Policy` | `accelerometer=(), autoplay=(), browsing-topics=(), camera=(), clipboard-read=(), clipboard-write=(), display-capture=(), fullscreen=(), geolocation=(), gyroscope=(), magnetometer=(), microphone=(), midi=(), payment=(), picture-in-picture=(), screen-wake-lock=(), usb=(), web-share=(), xr-spatial-tracking=()` | Powerful features the site never uses, so an injected script or a framed third party cannot ask for them. FR-D1's five are inside the list. | Not set. |
...
| `X-DNS-Prefetch-Control` | `off` | DNS prefetching leaking the hostnames of links and hot-linked images the page merely mentions, before a visitor ever follows one. | Not set. |
| `X-Permitted-Cross-Domain-Policies` | `none` | A legacy Flash or Acrobat plugin treating a cross-domain policy file served from this origin as permission to reach into responses here. | Not set. |
...
| `frame-ancestors` | `'none'` | The site is never framed. The interactive server render mode's default `'self'` is suppressed so this is the app's only frame-ancestors; a proxy copy, if one exists, also applies and the stricter wins. | No iframe or embed anywhere; the admin theme preview is inline markup. |
