...
**Grilling outcome (2026-09-13)**: the handoff and this plan were reviewed in a grilling session (fourteen decisions, all as recommended; requirements section 10). Changes to this plan: the gates collapse to one approval per unit (Unit 11: functional design, code-generation plan and the quick-fix plan in one stop; Unit 12: the three design documents and the plan in one stop; completions are reports; stops stay before each deploy and for the owner's admin-page check); a quick fix, PR 1.5 `fix: show the average per day on the daily visitors tile` (`construction/plans/quick-stats-tile-plan.md`), runs ahead of PR 2; the release and deploy happen after PR 3 (with `SECURITY_CSP_MODE=report-only` in production until the owner's check, then `enforce`) and after PR 4 (with `TRUSTED_PROXIES` and `WEB_BIND` set by the session). The PR 3 change map loses the nonce plumbing and gains the theme snapshot hash, the theme editor's CSSOM colors and forced reload after a save, and the removal of `<ImportMap />`; the PR 4 change map gains the path-scoped global limiter for the OAuth handler paths. Next: the Unit 11 stop.
